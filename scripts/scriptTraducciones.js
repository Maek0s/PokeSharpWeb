...
async function loadVersions(language) {
    ...
    if (pageTitle == "PokeSharp - Updates" || pageTitle == "PokeSharp - Actualizaciones") {
        try {
            const response = await fetch('/scripts/jsons/versions.json');  // Ruta al archivo JSON de versiones
            if (!response.ok) {
                throw new Error(`Error al cargar el archivo JSON de versiones: ${response.statusText}`);
            }
            ...
            console.error("Hubo un error cargando el archivo JSON de versiones: ", error);
        }
    }
}
